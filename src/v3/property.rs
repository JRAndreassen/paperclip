...
pub(crate) struct Property {
    ...
    all_vars: Vec<Property>,
    /// Only the required properties.
    required_vars: Vec<Property>,
    /// Only the optional properties.
    optional_vars: Vec<Property>,
    // Only the read-only properties.
    read_only_vars: Vec<Property>,
    // The read/write properties.
    read_write_vars: Vec<Property>,
    /// The Self's parent properties.
    parent_vars: Vec<Property>,
...
impl Property {
    ...
    fn post_process_refmut(&mut self) {
        let mut data_type = self.data_type.clone();
        ...
        Self::post_process_dt(&mut data_type, is_decl);
    ...
    pub fn with_parent(mut self, parent: &Option<&Self>) -> Self {
        self.parent = parent.map(|p| Box::new(p.clone()));
        self
    }
    ...
    fn with_obj(mut self, root: &super::OpenApiV3, by: &openapiv3::ObjectType) -> Self {
        ...
        self.discovered_props = vars.clone();
        let vars = vars
            .into_iter()
        ...
        if let Some(one_of) = &by.one_of {
            let mut vars_ = vars.iter().filter(|p| !p.required).collect::<Vec<_>>();
            if vars_.len() != vars.len() {

...
impl Operation {
    ...
    pub(crate) fn new(
        ...
        for param in &path_params {
            if param.data_format() == "url" {
                path = format!("{}:.*", param.base_name());
                vendor_extensions.insert("x-actix-query-string".into(), "true".into());
            }
        }
        vendor_extensions.insert("x-actixPath".into(), path.clone());
    ...
    /// Get a reference to the operation url path.
    pub fn path(&self) -> &str {
        &self.path
    }
    /// Get a reference to the map of vendor extensions.
    pub fn vendor_extensions(&self) -> &HashMap<String, String> {
